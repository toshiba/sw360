...
public class SW360ConfigDatabaseHandler {
    private final Logger log = LogManager.getLogger(this.getClass());
    private final ConfigContainerRepository repository;
    private static final Map<String, String> configMapInMem = new HashMap<>();

    public SW360ConfigDatabaseHandler(Supplier<CloudantClient> httpClient, String dbName) {
        DatabaseConnectorCloudant db = new DatabaseConnectorCloudant(httpClient, dbName);
        repository = new ConfigContainerRepository(db);
        try {
            loadToConfigMapInMem(repository.getByConfigFor(ConfigFor.SW360_CONFIGURATION));
        } catch (IllegalStateException exception) {
            log.error(exception.getMessage());
            loadToConfigMapInMem(null);
        }
    }

    private void loadToConfigMapInMem(ConfigContainer configContainer) {
        configMapInMem
    ...
    public RequestStatus updateSW360Configs(Map<String, String> updatedConfigs, User user) throws SW360Exception {
        ...
        loadToConfigMapInMem(configContainer);
        log.info(configMapInMem);
        return RequestStatus.SUCCESS;
    }


    public Map<String, String> getSW360Configs() {
        return configMapInMem;
    }

    public String getConfigByKey(String key) {
        return configMapInMem.get(key);
    }
}
