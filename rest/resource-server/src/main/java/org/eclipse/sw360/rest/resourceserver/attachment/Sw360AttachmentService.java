...
public class Sw360AttachmentService {
    ...
    public CollectionModel<EntityModel<AttachmentDTO>> getAttachmentDTOResourcesFromList(Set<AttachmentDTO> attachmentList) {
        final List<EntityModel<AttachmentDTO>> attachmentResources = new ArrayList<>();
        if (CommonUtils.isNotEmpty(attachmentList)) {
            for (final AttachmentDTO attachment : attachmentList) {
    ...
    public Map<Attachment, UsageAttachment> setAttachmentsInRequest(User user, Set<Attachment> attachments, Source owner) throws TTransportException {
        ...
        try {
            ...
            restrictedProjectsCountsByContentId.entrySet().stream().forEach(stringLongEntry -> {
                try {
                    List<AttachmentUsage> attachmentUsages1 = getAttachmentUseById(stringLongEntry.getKey());
                    Attachment attachment = getAttachmentForId(stringLongEntry.getKey());
                    UsageAttachment usage =  new UsageAttachment();
                    usage.setVisible(attachmentUsages1.size());
                    usage.setRetricted(stringLongEntry.getValue());

                    Set<String> projectNames = new HashSet<>();
                    attachmentUsages1.forEach(attachmentUsage -> {
                        try {
                            StringBuilder stringBuilder = new StringBuilder();
                            Project project = projectService.getProjectForUserById(attachmentUsage.getUsedBy().getProjectId(), user);
                            stringBuilder.append(project.getName());
                            stringBuilder.append("(");
                            stringBuilder.append(project.getVersion());
                            stringBuilder.append(")");

                            projectNames.add(stringBuilder.toString());
                        } catch (TException e) {
                            throw new RuntimeException(e);
                        }
                    });
                    usage.setProjectName(projectNames);
                    attachmentUsageMap.put(attachment,usage);
 //                    attachmentUsages.addAll(attachmentUsages1);
                } catch (TException e) {
                    throw new RuntimeException(e);
                }
            });
